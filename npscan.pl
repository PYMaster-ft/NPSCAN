...
#############################################################################################################################
## INTRODUCTION #############################################################################################################
#   This script is Copyright (c) 2021 NEO PROGRAMMERS see License.txt
#   [::] SCRIPT NAME:  npscan scanner
#   [::] AUTOR:        Ali Mehdioui founder of neo programmers
#   [::] FB:           https://facebook.com/neoprogrammers
#   [::] Twitter:      https://twitter.com/neoprogrammers
#   [::] GIT:          https://github.com/cham2003
#   [::] YOUTUBE:      http://youtube.com/c/neoprogrammers
#
## DESCRIPTION ##############################################################################################################
...
## DEFINE ARRAYS
my (@proxies, @exploits, @motors, @target_urls, @dorks, @targenPscans, $isscan);
...
for my $targ(@targets) {
  ...
  for (@target_urls) {
    if ($_ !~/(\=rang|\=repeat)/) {
	  ...
	  push @nPscans, $_ if $valido;
	  Print::Conclure($limit, $ifend, $isscan, $output, scalar @nPscans, scalar @targets) if $in eq $limit;
	}
  }
}

## END SCANS
Print::Conclure($limit, $ifend, $isscan, $output, scalar @nPscans, scalar @targets) if $limit eq 500;
