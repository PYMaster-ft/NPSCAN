...
sub uninstall {
  ...
  chomp ($resp);
  if ($resp=~/(Y|y)/) {
    my @uninstall=($scriptbash, $script, "$scriptCompletion/npscan", "/usr/share/applications/npscan.desktop");
    for (@uninstall) {
      print $c[10]."[i] $ZT[8] $_... ";
      unlink $_ if -e $_;
	...
	my @deleteIcons=( "/usr/share/icons/hicolor/16x16/apps/npscan-menu.png", 
	                  "/usr/share/icons/hicolor/22x22/apps/npscan-menu.png", 
					  "/usr/share/icons/hicolor/24x24/apps/npscan-menu.png", 
					  "/usr/share/icons/hicolor/48x48/apps/npscan-menu.png", 
					  "/usr/share/icons/hicolor/256x256/apps/npscan-menu.png"
					 );
    for (@deleteIcons) {
      print $c[10]."[i] $ZT[8] $_... ";
      unlink $_ if -e $_;
